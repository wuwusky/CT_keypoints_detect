...
class MobileNetV3(nn.Module):
    ...
    def __init__(
        ...
        self.attention_slice = nn.Sequential(
            nn.Flatten(1),
            nn.Linear(512, 128),
            nn.ReLU(True),
            nn.Linear(128, 1),
        ...
        self.feature_sclice = nn.Sequential(
           nn.Conv2d(lastconv_output_channels, 512, 3, 1, 1),
           nn.BatchNorm2d(512),
           nn.ReLU(True),
        ...
        self.feature_location = nn.Sequential(
            nn.Conv2d(lastconv_output_channels, 512, 3, 1, 1),
            nn.BatchNorm2d(512),
            nn.ReLU(True),
            nn.AdaptiveAvgPool2d(1),
        ...
        self.estimate_head_location = nn.Sequential(
            nn.Linear(512, 22),
            nn.Sigmoid(),
        )
        self.estimate_head_slice = nn.Sequential(
            nn.Linear(512, 11),
            nn.Sigmoid()
        )
    ...
    def forward_mil(self, x):
        ...
        features = self._feature_extract(x)

        slice_est = self.feature_sclice(features)
        slice_attention = self.attention_slice(slice_est)
        slice_attention = torch.transpose(slice_attention, 1, 0)
        location_est = self.feature_location(features)
        
        out_att = torch.mm(slice_attention, location_est)
        out_location = self.estimate_head_location(out_att)
        out_slice = self.estimate_head_slice(out_att)
        return slice_attention, out_location, out_slice
...
# temp_model = mobilenet_v3_small()
# temp_input = torch.randn(1,16,1,512,512)
# temp_out = temp_model.forward_mil(temp_input)
# print('test')
